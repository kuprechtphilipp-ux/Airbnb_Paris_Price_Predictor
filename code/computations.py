...
model_airbnb_price = pickle.load(open("code/ml_models/predict_airbnb_price.sav", "rb"))
model_cleaning_costs = pickle.load(open("code/ml_models/predict_cost_of_cleaning.sav", "rb"))
model_renting_price = pickle.load(open("code/ml_models/predict_renting_price.sav", "rb"))
...
def build_airbnb_feature_df(user_profile: dict) -> pd.DataFrame:
    ...
    df_features_airbnb.to_csv("code/data/user_dataset_airbnb.csv", index=False)
...
def build_renting_feature_df(user_profile: dict) -> pd.DataFrame:
    ...
    # debug: everytime i change something a dataset gets created and overwritten
    df_features_renting.to_csv("code/data/user_dataset_renting.csv",  encoding="utf-8-sig", index=False)
